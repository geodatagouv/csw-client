...
class Harvester extends Readable {
    ...
    init() {
        ...
        this.client.count(_.clone(this.options), (err, count) => {
            if (err) {
                debug('error in count operation');
    ...
    fetchRecords() {
        ...
        this.client.records(query, (err, result) => {
            if (err) {
                debug('error while fetching records from %d', query.startPosition);
                debug(JSON.stringify(err));
                this.emit('error', err);
            } else {
    ...
    fetchMoreRecords() {
        ...
        _.times(this.concurrency - this.pendingRequests, () => this.fetchRecords());
    }
