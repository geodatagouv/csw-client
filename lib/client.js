    ...
    decompressedResponse
      ...
      .on('result', result => {
        if (result.type === 'ExceptionReport') {
          ...
          const err = new Error(`ExceptionReport: ${exception.exceptionCode} (${exception.locator})`)
...
class Client extends EventEmitter {
  ...
  async records(options = {}) {
    const response = await this.getRecords(options)
    const result = await parseResponse(response)
  ...
  async count(options = {}) {
    ...
    const result = await this.records(recordOptions)
