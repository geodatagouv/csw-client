...
const { once, pick, defaults } = require('lodash');
...
class Client extends EventEmitter {
    ...
    count(options, done) {
        ...
        options = Object.assign({}, options, { resultType: 'hits' });
    ...
    record(id, options, done) {
        if (!done) {
            done = options;
            options = {};
        }
        parseResponse(this.getRecordById(id, options), function (err, result) {
            if (err) return done(err);
            if (result.type !== 'GetRecordByIdResponse') return done(new Error('Not acceptable response type: ' + result.type));
            if (result.body.children.length === 0) return done(new Error('No record returned'));
            done(null, result.body.children[0]);
        });
    }
