  ...
  async records(options = {}) {
    ...
    try {
      if (this.hasCompatOption('progressive-element-set-name')) {
        ...
        return {
          records: await Promise.all(records.map(record => this.record(record.originalId, recordOptions))),
          matched,
          returned
        }
      }
      ...
      if (error instanceof ExceptionReport) {
        const shouldRetry = detectCompatOptions(this, error.exception)
        if (shouldRetry) {
