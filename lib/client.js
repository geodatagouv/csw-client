...
Client.prototype.request = function(query, cb) {
    ...
    req.cookies = 'fix'; // Fix for geocatalogue.fr
    // May use req.unset('Cookie') in the future (superagent-retry / 0.3.1?)
    // May be dropped in the future (superagent / 0.19?)
    // Incoming PR https://github.com/segmentio/superagent-retry/pull/10
    // Incoming PR https://github.com/visionmedia/superagent/pull/382
...
Client.prototype.mapOptions = function(options) {
    ...
    _.extend(query, _.pick(options, 'maxRecords', 'startPosition', 'typeNames', 'outputSchema', 'elementSetName', 'resultType', 'namespace'));
