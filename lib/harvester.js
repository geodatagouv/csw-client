...
const { extendHarvestOptions } = require('./compatibility')
const { withSummary } = require('./records')
...
class Harvester extends Readable {
  ...
  constructor(client, options = {}) {
    ...
    this.client = client
    this.options = extendHarvestOptions(options)
    this.step = step
  ...
  init() {
    ...
    this.client.count(clone(this.options))
      .then(
        count => {
