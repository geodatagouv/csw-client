...
class Harvester extends Readable {
  ...
  nextPass() {
    if (!this.remainingPassSchema) throw new Error('No remaining pass')
  ...
  async fetchRecords() {
    ...
    if (this.currentPassComplete()) this.nextPass()
  ...
  hasMoreRecords() {
    ...
    if (this.remainingPassSchema || !this.currentPassComplete()) return true
